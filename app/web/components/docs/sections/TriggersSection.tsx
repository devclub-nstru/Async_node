      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-[13.5px] leading-relaxed text-white/55">
            Runs the workflow immediately when you click <span className="text-white/80">Run</span> in the trigger&apos;s config panel. No external input — downstream nodes see the trigger&apos;s stored config as-is.
          </p>
        </div>
        ...
        <div>
          ...
          <CodeBlock label="Webhook URL format" code={"POST {backend_URL}/v1/webhooks/{webhookToken}"} />
        ...
        <div>
          ...
          <p className="mt-1 text-[13.5px] leading-relaxed text-white/55">
            Set an interval in seconds (minimum 60) and click <span className="text-white/80">Start</span>. The workflow then runs automatically on that interval until you click <span className="text-white/80">Stop</span>. The panel shows whether the schedule is currently running.
          </p>
        </div>
      </div>

      <ImagePlaceholder label="Screenshot: trigger config panel showing Manual / Webhook / Schedule options" aspect="video" />
    </DocsSection>
  )
}
