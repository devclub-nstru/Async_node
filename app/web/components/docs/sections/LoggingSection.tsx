...
export default function LoggingSection() {
  return (
    ...
    >
      ...
      <div>
        ...
        <ul className="space-y-2 text-[13px] text-white/55">
          <li className="flex items-center gap-2">
            <Loader2 size={14} className="animate-spin text-amber-400" /> Running — the node is currently executing
          </li>
          <li className="flex items-center gap-2">
            <Check size={14} className="text-emerald-400" /> Success — the node completed without errors
          </li>
          <li className="flex items-center gap-2">
            <X size={14} className="text-red-400" /> Failed — the node threw an error; expand the row to see the message
          </li>
        </ul>
      </div>
      ...
      <ImagePlaceholder label="Screenshot: Run Log tab with a mix of success and failed nodes, one row expanded" aspect="video" />
      <ImagePlaceholder label="Screenshot: Selected Node tab showing a node's raw output" aspect="video" />
    </DocsSection>
  )
}
